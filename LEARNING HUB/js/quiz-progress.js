...
window.addEventListener("message", async (event) => {
  ...
  if (!trustedOrigin || !messageTypes.has(message?.type)) return;
  ...
  if (!requestId || !safeIdPattern.test(contentId)) {
    reply(event.source, requestId, { ok: false, code: "quiz-progress/invalid-request" });
    return;
  }
  if (!uid) {
    reply(event.source, requestId, { ok: false, code: "quiz-progress/sign-in-required" });
    return;
  }
  ...
  try {
    ...
    reply(event.source, requestId, { ok: true, action: "save" });
  } catch (error) {
