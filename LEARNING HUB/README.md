...
18. `public/shared/learning-hub-tools.js` — ตัวกันหน้าต่าง Copy และปุ่มย้อนกลับกลางสำหรับ Hub/Quiz/Simulation/Notebook
...
## การเรียกเครื่องมือกลางจาก Quiz

เพิ่มบรรทัดนี้ก่อน `</body>` ใน Quiz หรือ Simulation ที่ต้องการเชื่อมกับ Hub:

```html
<script src="https://nixcentury.github.io/Physic-subject/shared/learning-hub-tools.js" data-hub-back defer></script>
```

บรรทัดเดียวจะปิดหน้าต่างแตะค้าง/คัดลอกและเพิ่มปุ่มย้อนกลับให้อัตโนมัติ ช่อง `input`, `textarea`, `select`, `contenteditable` และส่วนที่ใส่ `data-allow-selection` ยังเลือกและคัดลอกข้อความได้ตามปกติ เมื่อเปิดงานในหน้าต่าง Hub ปุ่มย้อนกลับจะปิดหน้าต่างงานนั้น แต่เมื่อเปิด Quiz โดยตรงจะย้อนกลับหน้าก่อนหรือกลับหน้า Hub
...
- เครื่องมือกลางหนึ่งไฟล์พร้อมให้ Quiz/Simulation ชุดใหม่เรียกตัวกัน Copy และปุ่มย้อนกลับได้
